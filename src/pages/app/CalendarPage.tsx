...
import { appointmentService, appointmentStatus, formatTime } from '../../lib/format'
...
export function CalendarPage() {
  ...
  const dayAppointments = useMemo(() => {
    ...
    return rows
      .filter((appointment) => isSameDay(new Date(appointment.starts_at), selectedDate))
      .filter((appointment) => providerId === 'all' || appointment.professional_id === providerId)
      .filter((appointment) => statusFilter === 'all' || appointment.status_code === statusFilter)
  ...
  if (!appointments.data && appointments.error) {
    return <ErrorState description={appointments.error.message} onRetry={appointments.retry} />
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex gap-2 overflow-x-auto pb-1">
        <FilterChip active={providerId === 'all'} label="All providers" onClick={() => setProviderId('all')} />
      ...
      <div className="flex items-center justify-between">
        ...
        <Link className="text-xs font-semibold text-primary" to="/app/appointments/new">
          Create appointment
        </Link>
      </div>
        ...
        <Card padding="sm">
          <div className="space-y-1">
            {dayAppointments.map((appointment, index) => (
              <AppointmentRow appointment={appointment} index={index} key={appointment.id} />
            ))}
          </div>
        </Card>
      ) : (
        <EmptyState description="No services are scheduled for this date." title="Calendar is clear" />
      )}
    </div>
  )
}
...
function AppointmentRow({ appointment, index }: { appointment: Appointment; index: number }) {
  ...
  return (
    <Link className="grid grid-cols-[52px_1fr] gap-3" to={`/app/appointments/${appointment.id}`}>
      <span className="pt-3 text-[11px] font-semibold text-muted">{formatTime(appointment.starts_at)}</span>
      <div className={`rounded-md border-l-4 p-3 ${toneClass}`}>
        <div className="flex justify-between gap-2">
          <div>
            <p className="text-xs font-semibold">{appointment.client_name ?? 'Client'}</p>
            <p className="mt-0.5 text-[11px] text-muted">{appointmentService(appointment)}</p>
            <p className="mt-1 text-[10px] font-semibold text-[#7344cd]">{appointmentStatus(appointment.status_code)}</p>
          </div>
          <Avatar name={appointment.professional_name ?? 'Professional'} size="sm" />
        </div>
      </div>
    </Link>
  )
