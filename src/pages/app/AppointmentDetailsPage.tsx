...
export function AppointmentDetailsPage() {
  ...
  return (
    <div className="space-y-5">
      ...
      <PageTitle title="Booking details" subtitle="Review and manage this salon appointment." />

      <Card className="text-center" tone="lavender">
        <Avatar className="mx-auto" name={data.client_name ?? 'Client'} size="lg" />
        <h2 className="mt-3 text-lg font-semibold">{data.client_name ?? 'Client'}</h2>
        <p className="text-xs text-muted">{appointmentService(data)}</p>
        <Badge className="mt-3" tone={tone}>{status}</Badge>
      </Card>

      <ScreenSection title="Schedule">
        <Card className="space-y-4">
          <Detail icon={<CalendarDays />} label="Date" value={formatDate(data.starts_at)} />
          <Detail icon={<Clock3 />} label="Time" value={formatTime(data.starts_at)} />
          <Detail icon={<UserRound />} label="Professional" value={data.professional_name ?? 'Professional'} />
          <Detail icon={<DollarSign />} label="Service cost" value={formatMoney(service?.unit_price_minor ?? 0)} />
          <Detail icon={<MapPin />} label="Location" value="Glow Salon" />
        </Card>
      </ScreenSection>
      ...
      <div className="grid gap-3">
        ...
        {data.status_code === 'coming_up' && (
          <Button fullWidth loading={mutation.loading} onClick={() => updateStatus('completed')}>Complete service</Button>
        )}
...
function Detail({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return (
    <div className="flex items-center gap-3">
      <span className="grid size-9 place-items-center rounded-md bg-lavender text-primary [&>svg]:size-4">{icon}</span>
      <div>
