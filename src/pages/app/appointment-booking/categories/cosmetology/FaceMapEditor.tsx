...
export function FaceMapEditor({
  ...
  return (
    <div
      className="flex w-full max-w-[393px] flex-col gap-6"
      data-node-id="662:8803"
    >
      <p className="text-[16px] leading-[1.4] text-[#475467]">
        Select an alteration type, then draw over the affected areas on the
        face.
      </p>
          ...
          {cosmetologyAnnotationTypes.map((option) => (
            <button
              className={cn(
                ...
                type === option
                  ...
                  : "bg-white text-[#475467] ring-1 ring-[#d0d5dd]"
              )}
              key={option}
            ...
            >
              <span
                ...
                style={{
                  backgroundColor: annotationColors[option] ?? "#7344cd",
                }}
              />
              {option}
            </button>
      ...
      <label className="flex w-full flex-col gap-2">
        ...
        <textarea
          ...
          onChange={(event) =>
            onChange({ ...details, skinAlterationNotes: event.target.value })
          }
      ...
      <div className="grid grid-cols-2 gap-2">
        <Button
          onClick={() =>
            onChange({ ...details, faceAnnotations: annotations.slice(0, -1) })
          }
          type="button"
          variant="outline"
        >
