...
import { clearSignUpDraft, readSignUpDraft } from './signUpDraft'
...
export function RegistrationPage() {
  ...
  const errors = useMemo<RegistrationErrors>(() => {
    ...
    return nextErrors
  }, [documentFile, salonLocation, selectedPlaceId, submitted])
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!documentFile || !salonLocation.trim() || (googleMapsApiKey && !selectedPlaceId)) {
      return
    }

    const draft = readSignUpDraft()
    if (!draft.salonName.trim() || !draft.acceptedTerms) {
      setRegistrationError('Please complete the sign up form before submitting verification.')
      return
    }

    if (draft.authProvider === 'email' && (!draft.email.trim() || !draft.password)) {
      setRegistrationError('Please complete the sign up form before submitting verification.')
      return
    }

    if (draft.authProvider === 'google' && !draft.googleCredential) {
    ...
    try {
      ...
      const result = draft.authProvider === 'google'
        ? await glamhourApi.registerGoogleSalon({
          ...verificationInput,
          credential: draft.googleCredential,
        })
        ...
        <div className="w-full rounded-xl bg-[#f7f8ff] px-5 py-8 shadow-[0_18px_42px_rgb(68_43_132_/_0.25)] ring-1 ring-white/75">
          <header className="text-center">
            ...
            <p className="mt-1 text-[11px] text-[#7c8394]">
              {state.salonName ? `Verify ${state.salonName} ownership` : 'Verify your salon ownership'}
            </p>
          </header>
